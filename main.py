...
from playsound import playsound  # Import for playing the alert sound
from threading import Thread
from Pose_Detection import mp_pose, pose, detect_action  # Import the functions
#ALERT_SOUND_PATH = r"C:\Users\Ramya Anugu\Downloads\WhatsApp Audio 2025-03-03 at 7.58.58 PM.mpeg"

# Path to the video file
path1 = r"C:\Users\Ramya Anugu\Downloads\1103313421-preview.mp4"
#def play_alert_sound():
    #playsound(ALERT_SOUND_PATH)

# Function to send alert and play sound
def send_alert_with_sound(frame, message):
    send_telegram_alert(frame, message)  # Send the alert
    #
...
try:
    while True:
        ...
        for i, (objectID, centroid) in enumerate(objects.items()):
            ...
            x1, y1 = max(0, x1), max(0, y1)  # Prevent out-of-bounds errors
            ...
            # Provide image dimensions to MediaPipe Pose
            pose_results = pose.process(cv2.cvtColor(person_image, cv2.COLOR_BGR2RGB))
            if pose_results.pose_landmarks:
                action = detect_action(pose_results.pose_landmarks)
                print(f"Person {objectID} Action: {action}")
                if action:
                    send_alert_with_sound(frame, action)
                cv2.putText(frame, action, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
                    ...
                    if results.face_landmarks:
                        ...
                        if face_class=="Distress" or face_class=="Fear":
                            ...
                            if gender_label[0]=="female":    
                                send_alert_with_sound(frame,"stress")
        ...
        if female_count == 1 and male_count == 0:
            send_alert_with_sound(frame, "Female detected alone at night!")
        ...
        if female_count > 0 and n > 0:
            try:
                ...
                if process_frame(frame, female_bboxes, male_bboxes):
                    send_alert_with_sound(frame, "Female surrounded by men, potential danger detected!")
